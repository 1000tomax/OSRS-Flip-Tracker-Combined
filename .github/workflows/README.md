...
- **Exemptions**: Issues/PRs labeled `pinned`, `security`, or `bug` are never marked stale
...
1. **Check the logs**: Click on failed workflow → Click on failed job → Review logs
2. **Common issues**:
...
- Check workflow syntax with [workflow syntax validator](https://github.com/actions/workflow-parser)
