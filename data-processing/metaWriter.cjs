...
async function writeMeta() {
  ...
  console.log(`✅ Generated meta: ${totalFlips} flips, ${totalProfit} GP profit`);
}

async function writeSummaryIndex() {
  const summaryDir = path.join(__dirname, '..', 'public', 'data', 'daily-summary');
  let entries;
  try {
    entries = await fs.promises.readdir(summaryDir, { withFileTypes: true });
  } catch (err) {
    const outPath = path.join(__dirname, '..', 'public', 'data', 'summary-index.json');
  ...
  console.log(`✅ Generated summary index: ${dates.length} dates`);
}
