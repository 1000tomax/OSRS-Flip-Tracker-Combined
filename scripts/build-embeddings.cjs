...
function loadRows(files) {
  ...
  for (const file of files) {
    ...
    if (DEBUG) console.log(`[build-embeddings] ${path.relative(ROOT, file)} -> ${part.length} rows`);
    rows.push(...part);
  }
  return rows;
}
...
for (const r of rows) {
  ...
  const marginEach = Number.isFinite(profit) && Number.isFinite(closedQty) && closedQty > 0
    ? profit / closedQty
    : NaN;
...
console.log(`[build-embeddings] ${out.length} items → ${path.relative(ROOT, OUT)} (from ${sources.length} file(s))`);
