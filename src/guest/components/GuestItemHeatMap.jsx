...
export default function GuestItemHeatMap({ itemName, guestData, metric = 'profit', minFlipsThreshold = 2, onCellClick }) {
  const cells = useMemo(() => getItemTimePatterns(itemName, guestData?.flipsByDate), [itemName, guestData]);

  const [selectedMetric, setSelectedMetric] = useState(metric);

  const maxValues = useMemo(() => {
    let maxProfit = 0, maxFlips = 0, maxAvg = 0;
    cells.forEach(c => {
  ...
  const grouped = useMemo(() => {
    ...
    cells.forEach(c => { g[c.day][c.hour] = c; });
    return g;
  }, [cells]);
    ...
    <div className="bg-gray-800 p-6 rounded-lg">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 gap-3">
        <div>
          ...
          <p className="text-xs text-gray-400 mt-1">{itemName} • Hours in {userTimezone}</p>
        </div>
      ...
      <div className="overflow-auto">
        <table className="w-full text-xs">
          <thead>
            <tr>
              <th className="px-2 py-1 text-left text-gray-300 sticky left-0 bg-gray-800">Day/Hour</th>
              {Array.from({ length: 24 }, (_, h) => (
                <th key={h} className="px-2 py-1 text-center text-gray-300">{formatHour(h)}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {grouped.map((row, dayIdx) => (
              <tr key={dayIdx}>
                <td className="px-2 py-1 text-gray-300 sticky left-0 bg-gray-800">{dayNames[dayIdx]}</td>
                {row.map((cell, hourIdx) => {
                  const value = !cell ? 0 : selectedMetric === 'flips' ? cell.flips : selectedMetric === 'avg' ? (cell.flips >= minFlipsThreshold ? cell.avgProfit : 0) : cell.profit;
                  const maxRef = selectedMetric === 'flips' ? maxValues.maxFlips : selectedMetric === 'avg' ? maxValues.maxAvg : maxValues.maxProfit;
                  const color = getIntensityColor(value, maxRef);
                  const tooltip = !cell ? 'No data' : `${dayNames[dayIdx]} ${formatHour(hourIdx)}\nFlips: ${cell.flips}\nProfit: ${formatGP(cell.profit)}\nAvg: ${formatGP(cell.avgProfit)}`;
                  return (
                    <td key={hourIdx} className="p-0.5">
                      <button
                        title={tooltip}
                        onClick={onCellClick ? () => onCellClick({ day: dayIdx, hour: hourIdx, ...cell }) : undefined}
      ...
      <div className="mt-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
          {topSlots.map((c, idx) => (
            <div key={`${c.day}-${c.hour}-${idx}`} className="bg-gray-700/60 rounded p-2 text-gray-200">
              <div className="text-xs text-gray-300">{dayNames[c.day]} • {formatHour(c.hour)}</div>
              <div className="text-xs">Flips: <span className="text-gray-100">{c.flips}</span></div>
              <div className="text-xs">Profit: <span className={c.profit>=0 ? 'text-green-300' : 'text-red-300'}>{formatGP(c.profit)}</span></div>
              <div className="text-xs">Avg/Flip: <span className="text-gray-100">{formatGP(c.avgProfit)}</span></div>
            </div>
          ))}
        </div>
      </div>
