...
export default function GuestItemTimeBars({ itemName, guestData, defaultMetric = 'profit' }) {
  ...
  const hourly = useMemo(() => {
    ...
    arr.forEach(a => { a.avg = a.flips > 0 ? a.profit / a.flips : 0; });
    return arr;
  }, [cells]);
    ...
    <div className="bg-gray-800 p-6 rounded-lg">
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex gap-2">
          <button onClick={() => setMetric('profit')} className={`px-3 py-1 text-sm rounded ${metric==='profit'?'bg-blue-600 text-white':'bg-gray-700 text-gray-300 hover:bg-gray-600'}`}>Profit</button>
          <button onClick={() => setMetric('flips')} className={`px-3 py-1 text-sm rounded ${metric==='flips'?'bg-blue-600 text-white':'bg-gray-700 text-gray-300 hover:bg-gray-600'}`}>Flips</button>
          <button onClick={() => setMetric('avg')} className={`px-3 py-1 text-sm rounded ${metric==='avg'?'bg-blue-600 text-white':'bg-gray-700 text-gray-300 hover:bg-gray-600'}`}>GP/Flip</button>
        </div>
      </div>
        ...
        <div className="h-56">
          ...
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={hourly} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
              ...
              <XAxis dataKey="hour" stroke="#9CA3AF" fontSize={12} tickFormatter={(h)=>h.toString()} />
              <YAxis stroke="#9CA3AF" fontSize={12} tickFormatter={yTick} />
              <Tooltip contentStyle={{ backgroundColor: '#1F2937', border: '1px solid #374151', borderRadius: 8, color: 'white' }} formatter={(v)=> metric==='flips' ? [v, 'Flips'] : [formatGP(v), metric==='avg'?'GP/Flip':'Profit']} />
        ...
        <div className="h-56">
          ...
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={daily} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#374151" />
              <XAxis dataKey="day" stroke="#9CA3AF" fontSize={12} tickFormatter={(d)=>dayNamesShort[d]} />
              <YAxis stroke="#9CA3AF" fontSize={12} tickFormatter={yTick} />
              <Tooltip contentStyle={{ backgroundColor: '#1F2937', border: '1px solid #374151', borderRadius: 8, color: 'white' }} formatter={(v)=> metric==='flips' ? [v, 'Flips'] : [formatGP(v), metric==='avg'?'GP/Flip':'Profit']} />
              <Bar dataKey={barKey} fill={barColor} />
            </BarChart>
