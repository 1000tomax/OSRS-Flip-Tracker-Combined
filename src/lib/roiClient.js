...
const ABS_CAP = 0.35;                // absolute guardrail ±35%
...
export async function loadROIClientSide() {
  ...
  if (idx && Array.isArray(idx.days) && idx.days.length) {
    ...
    for (const d of days) {
      const net = num(d.netWorth ?? d.net ?? d.net_worth);
      const profit = num(d.profit ?? d.gp_per_day);
      let r = num(d.roi_decimal);

      if (!Number.isFinite(r)) {
        let pc = num(d.percentChange ?? d.pctChange ?? d["%Change"] ?? d["% change"] ?? d.roi_percent);
        if (Number.isFinite(pc)) r = Math.abs(pc) > 1 ? pc / 100 : pc;
      }
      if (!Number.isFinite(r) && Number.isFinite(profit) && Number.isFinite(prevNet)) {
        r = profit / Math.max(prevNet, 1);
      }

      if (Number.isFinite(r) && Number.isFinite(prevNet) && prevNet >= MIN_NET_FOR_ROI) {
        roiRaw.push(r);
        if (!firstUsedDate) firstUsedDate = d.date;
      }

      if (Number.isFinite(net)) prevNet = net;
    }
  ...
  // CSV fallback if needed (unchanged)
  ...
  if (t) {
    ...
    if (pct.filter(Number.isFinite).length >= 5) {
      ...
      for (let i = 1; i < nets.length; i++) {
        const prior = Math.max(1, nets[i-1] || 1);
        roi.push((profits[i] || 0) / prior);
      }
    }
  ...
  // Last resort demo
  return { roi: [0.01,0.006,-0.004,0.012,0.007,0.004,0.013,-0.006,0.009,0.005], startNet: 25_000_000, stats: { n: 10 } };
}