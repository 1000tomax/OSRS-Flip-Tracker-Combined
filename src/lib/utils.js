...
export function isIncompleteDay(day, allDays) {
  ...
  try {
    ...
    const maxDay =
      allDays && allDays.length > 0 ? Math.max(...allDays.map(d => d.day || 0)) : day.day || 0;
...
export function getCategoryFromName(name = '') {
  ...
  if (s.includes('helm') || s.includes('plate') || s.includes('legs') || s.includes('kite'))
    return 'Armor';
  if (s.includes('sword') || s.includes('scim') || s.includes('whip') || s.includes('bow'))
    return 'Weapons';
