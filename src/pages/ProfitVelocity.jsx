...
export default function ProfitVelocity() {
  ...
  React.useEffect(() => {
    logger.debug('Performance page - Data state:', {
  ...
  const velocityData = useMemo(() => {
    ...
    const flipsWithDuration = allFlips
      .map(flip => {
        try {
    ...
    const todayString = DateUtils.getTodayChicago();
  ...
  // Debug velocityData
  React.useEffect(() => {
    logger.debug('Performance page - Velocity data:', {
              ...
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={velocityData}>
                  ...
                  <XAxis
                    ...
                    height={60}
                  />
                  <YAxis stroke="#9CA3AF" tickFormatter={formatGP} />
                  <Tooltip
                    contentStyle={{
                      ...
                      return [
                        name === 'winRate'
                          ...
                          : name === 'flipCount'
                            ? value
                            : formatGP(value),
                        name === 'winRate' ? 'Profitable %' : name === 'flipCount' ? 'Flips' : name,
                      ];
                    }}
              ...
              <ChartErrorBoundary>
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={velocityData}>
                    ...
                    />
                    <Bar dataKey="flipCount" fill="#8B5CF6" radius={[4, 4, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              </ChartErrorBoundary>
            </div>
